...
def create_task(task_text, task_description, finished_date, creation_date, is_finished, important, progress):
    return Task.objects.create(task_text=task_text, task_description=task_description, finished_date=finished_date, creation_date=creation_date, is_finished=is_finished, important=important, progress=progress)
...
class CreateTaskTests(TestCase):
    ...
    def test_basictask(self):
        ...
        time = timezone.now()
        create_task("test", "testi", time, time, False, False, 12)
        response = self.client.get(reverse('tasks:index'))
